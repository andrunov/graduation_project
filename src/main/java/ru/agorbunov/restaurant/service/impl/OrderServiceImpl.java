...
public class OrderServiceImpl implements OrderService {
    ...
    *  update totalOrdersAmount in corresponding user-entity in success case*/
    ...
    private void checkPreviousOrders(int userId, Order order) {
        ...
        for (Order previousOrder : ordersForSameDay){
            if (!previousOrder.equals(order)){//for except case that order is not created but updated
                if (localDateTime.getHour()>11) throw new RefuseToUpdateException(String.format("order %s has made after 11:00, it is too late, vote can't be changed",order));
                else orderRepository.delete(previousOrder.getId());
            }
        }
    }
}
